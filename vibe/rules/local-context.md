...
本仓库是 `CzzRef/orca` 的本机 GitFork 检出，上游为 `stablyai/orca`。本地开发主分支是 `czz-dev`。GitHub Actions 已全部停用。不要复述 CodeNote 规则正文。
...
硬边界：不要建空的 `vibe/ai-db/` 或 `vibe/requirements/`；上游 PR 不要带 `vibe/`；未授权不要对用户桌面抢焦点、真实网络 `orca serve`、签名发布或 computer-use 真实已登录会话。
